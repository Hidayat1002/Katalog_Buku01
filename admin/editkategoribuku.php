...
if(isset($_GET['data'])){
  ...
  $sql_d = "select `kategori_buku` from `kategori_buku` where `id_kategori_buku` = '$id_kategori_buku'";
    ...
    while($data_d = mysqli_fetch_row($query_d)){$kategori_buku= $data_d[0];}
}
?>
      ...
      <form class="form-horizontal" method="post" action="konfirmasieditkategoribuku.php">
        <div class="card-body">
          <div class="form-group row">
            ...
            <div class="col-sm-7">
              <input type="text" class="form-control" name="kategori_buku" id="Kategori Buku" value="<?php echo $kategori_buku;?>">
            </div>
          </div>
        </div>
        <!-- /.card-body -->
